...
void solve()
{
	...
	auto dijkstra = [&](int src)
	{
		...
		while (!edg.empty())
		{
			...
			for (auto p : adj[from])
			{
				int to = p.first, wt = p.second;
				
				if (dist[to] > dist[from] + wt)
				{
					edg.erase({dist[to], to});
					dist[to] = dist[from] + wt;
					edg.insert({dist[to], to});
				}
			}		
		}
	...
	auto get = [&](int i, int j)
	{
		...
		for (auto route : routes)
		{
			...
			ans += min({d[from][to], d[from][i] + d[j][to], d[from][j] + d[i][to]});
		}	
	...
	for (auto edge : edges)
		cost = min(cost, get(edge[0], edge[1]));
